...
import requests
import json
#from sagenkarta_rest_api.tests.models import PageRecord, build_data

import logging
logger = logging.getLogger(__name__)

"""
Test case page-by-page using item "struct" class with test parameters for transcibe page-by-page 
TODO: Not working yet
"""

@dataclass
class PageRecord:
    record_id_page_by_page: str
    page_file_to_transcribe: str
    from_email: str
    from_name: str
    title: str
    message: str
    messageComment: str
    pagenumber: str
    fonetic_signs: str  # or bool if you prefer
    unreadable: str     # or bool if you prefer


def build_data(item, transcribesession, logid, timestamp_now):
    """
    all_data = [
        build_data(item, transcribesession, logid, timestamp_now)
        for item in items  # items = list of your objects
    ]
    """
    # logid = "build_data"
    print(logid + " item: " + str(item))
    # Always-present part
    data = {
        "transcribesession": transcribesession,
    }

    # Optional values mapped from the item
    optional_fields = {
        "recordid": getattr(item, "record_id_page_by_page", None),
        "page": getattr(item, "page_file_to_transcribe", None),
        "from_email": getattr(item, "from_email", None),
        "from_name": getattr(item, "from_name", None),
        "title": getattr(item, "title", None),
        "message": getattr(item, "message", None),
        "messageComment": getattr(item, "messageComment", None),
        "pagenumber": getattr(item, "pagenumber", None),
        "fonetic_signs": getattr(item, "fonetic_signs", None),
        "unreadable": getattr(item, "unreadable", None),
    }

    # Only keep values that are actually set
    for key, value in optional_fields.items():
        print(logid + " key: " + str(key) + " " + str(value))
        # adjust this condition to your taste:
        if value not in (None, ""):
            # Special handling for message: add logid + timestamp
            if key == "message":
                value = f"{value}{logid} {timestamp_now}"
            data[key] = value
...
class APITranscribeViewTestCasePageByPage(unittest.TestCase):
    ...
    record_id_page_by_page = "10789_X_27860"
    ...
    records: list[PageRecord] = [
        PageRecord(
            record_id_page_by_page="10789_X_27860",
            page_file_to_transcribe="uppteckningar/ifgh_00700-00799/ifgh00702_0002.jpg",
            from_email="pertest@isof.se",
            from_name="pertest",
            title=None,
            #title="Fastlag och påsk",
            message="Fastlag och påsk. Svar på .. ",
        ...
        PageRecord(
            record_id_page_by_page="ifgh00702_X_195386",
            page_file_to_transcribe="uppteckningar/ifgh_00700-00799/ifgh00702_0003.jpg",
            from_email="pertest@isof.se",
            from_name="pertest",
            title=None,
            message="Nu är påsken den glada tid .. ",
            messageComment="Fritext Svår stil",
            pagenumber=None,
            fonetic_signs=None,
            unreadable=None,
        ),
        PageRecord(
            record_id_page_by_page="ifgh00702_X_195386",
            page_file_to_transcribe="uppteckningar/ulma_10700-10799/10789_0002.jpg",
            from_email="pertest@isof.se",
            from_name="pertest",
            title="Fastlag och påsk",
            message="Nu är påsken den glada tid .. ",
            messageComment="Uppteckningsblankett Svår stil",
            pagenumber=None,
            fonetic_signs=None,
            unreadable=None,
        ),
    ...
    @unittest.skip("OLD: Only one record")
    def test_99_transcribe_page(self):
    ...
    def test_01_transcribe_page(self):
        ...
        for item in self.records:
            print(logid + " item: " + str(item))
            data = build_data(item, self.transcribe_session, logid, timestamp_now)
            print(logid + " data: " + str(data))
