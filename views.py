...
class DescribeViewSet(viewsets.ViewSet):
    """
    Handle descriptions for files in records_media
    ...
    def change(self, request):
        ...
        Parameters:
        Terms: Terms in request should be all terms:
            No need to validate that a term does not already exists
        """
                    ...
                    if str(record.transcriptiondate.strftime('%Y-%m-%d %H:%M:%S')) in transcribesession:
                        ...
                        if start_time is not None:
                            ...
                            if not time_exists:
                                ...
                                # Terms in request should be all terms:
                                # No need to validate that a term does not already exists
...
class UtterancesViewSet(viewsets.ViewSet):
    """
    Handle utterances for files in records_media
    Currently only descriptions for files of type audio
    """
    authentication_classes = (CsrfExemptSessionAuthentication, BasicAuthentication)
    # permission_classes = [AllowAny]  # This allows any user to access the endpoint
